...
class UserModel extends DB
{
    ...
    {
        $sql = "INSERT INTO users (fullname, email, password, tel, address,idGroups) values('$name','$email','$password','$tel','$address',2)";
    ...
    function editUser($id, $name, $img, $email, $address, $password, $date, $tel, $groups)
    {
        ...
        if (!empty($img) && empty($password)) {
            $sql = "UPDATE users SET fullname='$name', avt='$img', email='$email' , address='$address', updated_at='$date', tel='$tel', idGroups='$groups'   WHERE id='$id' ";
        } else {
            $sql = "UPDATE users SET fullname='$name', email='$email' , address='$address',  updated_at='$date', tel='$tel', idGroups='$groups'   WHERE id='$id' ";
        }
