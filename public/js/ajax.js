  ...
  const renderProduct = (item, img_path, url) => {
    ...
    <div class="item new-arrival-item shadow " data-id=" <?php echo $new['id'] ?>">
        ...
        <div class="arrival-product-content">
            <h3><a class="dot_hidden" href="http://localhost/QTDL-main/Detail/index/${item.id}">${item.name}</a></h3>
            <div class="arrival-product-evaluate">
            <div class="flex items-center justify-center gap-2 arrival-product-evaluate">

            <div class="rateYoSumpro" data-rateyo-read-only="true" data-rateyo-rating="${item.caculate_start}"></div>
             
                <span class="sold-out">${item.caculate_start}</span>
                </div>
            </div>
